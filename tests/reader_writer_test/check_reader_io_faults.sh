...
# always fail on first read: FASTA/FASTQ type detection
fiu-run -x -e posix/io/rw/read $PROG -i long.fa > /dev/null


# Fail 10% of the times - should happen in multiple stages of the FASTA reader
for i in $(seq 1 10) ; do
	echo "$i"
	fiu-run -x -e posix/io/rw/read -p 10 $PROG -i long.fa > /dev/null
	fiu-run -x -e posix/io/rw/read -p 10 $PROG -i long.fq > /dev/null
done
